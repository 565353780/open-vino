...
void ObjectDetector::fetchResults() {
	if (USE_OPENPOSE)
	{
		...
		int frame_len = json_reader->intlen(frame_idx_);
		...
		for (int det_id = 0; det_id < people_bboxs.size(); ++det_id) {

			const float score = people_bboxs[det_id][4];
			const float x0 = people_bboxs[det_id][0];
			const float y0 = people_bboxs[det_id][1];
			const float x1 = people_bboxs[det_id][2];
			const float y1 = people_bboxs[det_id][3];
