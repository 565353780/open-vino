...
public:
    ...
    void Compute(const cv::Mat &mat, cv::Mat *descr) override {
        ...
        PT_CHECK(!mat.empty());
		std::cout << "descriptor.hpp/Compute resize : " << descr_size_.width << "," << descr_size_.height << std::endl;
        cv::resize(mat, *descr, descr_size_, 0, 0, interpolation_);
		cv::Mat descr_show;
		cv::resize(*descr, descr_show, cv::Size(0, 0), 10, 10, interpolation_);
		cv::imshow("fast_descriptor", descr_show);
    }
    ...
    void Compute(const std::vector<cv::Mat> &mats,
		...
		std::cout << "descriptor.hpp/Compute multi resize : " << descr_size_.width << "," << descr_size_.height << std::endl;
