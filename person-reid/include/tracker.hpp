...
struct Track {
    ...
    Track(const TrackedObjects &objs, const cv::Mat &last_image,
        ...
        lost(0),
		max_episodes_to_show_predict(0),
        length(1) {
            PT_CHECK(!objs.empty());
            first_object = objs[0];
        }
	...
	size_t max_episodes_to_show_predict;
...
public:
	...
	///
	///
	void DrawDesciptorDiagram(cv::Mat &outImage, cv::Mat &descriptor);
...
private:
	...
	void ComputeStrongDesciptors(const cv::Mat &frame,
								 const TrackedObjects &detections,
								 std::vector<cv::Mat> *desriptors);
	...
	float AffinityStrong(const cv::Mat &descriptor1, const TrackedObject &obj1,
					   const cv::Mat &descriptor2, const TrackedObject &obj2);
...
public:
	bool USE_NETWORK_FOR_REID = false;
	bool SHOW_TRACK_LINES = true;
	bool use_det_bbox = false;
	int target_person_num = 0;
	iou* iou_;
	cv::Size frame_size;
