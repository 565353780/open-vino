...
void ObjectDetector::fetchResults() {
	if (USE_OPENPOSE)
	{
		...
		int frame_now = frame_idx_ - 1;

		int frame_len = json_reader->intlen(frame_now);

		stringstream ss;
		
		ss << "../../video/2019_2person_cut_json/2019_2person_cut_";
		...
		vector<int> keypoints_idx_det({5, 2, 9, 12});

		vector<vector<vector<double>>> peoples_bboxs;

		peoples_bboxs = json_reader->get_bbox_xyxy(people_keypoints, conf_thresh, expand_coefficient, det_expand_coefficient, keypoints_idx_used, keypoints_idx_det);

		for (int det_id = 0; det_id < peoples_bboxs.size(); ++det_id) {

			const float score = peoples_bboxs[det_id][0][4];
			const float x0 = peoples_bboxs[det_id][0][0];
			const float y0 = peoples_bboxs[det_id][0][1];
			const float x1 = peoples_bboxs[det_id][0][2];
			const float y1 = peoples_bboxs[det_id][0][3];
			const float det_x0 = peoples_bboxs[det_id][1][0];
			const float det_y0 = peoples_bboxs[det_id][1][1];
			const float det_x1 = peoples_bboxs[det_id][1][2];
			const float det_y1 = peoples_bboxs[det_id][1][3];
			...
			object.det_rect = cv::Rect(cv::Point(static_cast<int>(round(static_cast<double>(det_x0))),
				static_cast<int>(round(static_cast<double>(det_y0)))),
				cv::Point(static_cast<int>(round(static_cast<double>(det_x1))),
					static_cast<int>(round(static_cast<double>(det_y1)))));


			object.det_rect = TruncateToValidRect(IncreaseRect(object.det_rect,
				config_.increase_scale_x,
				config_.increase_scale_y),
				cv::Size(static_cast<int>(width_), static_cast<int>(height_)));

			object.frame_idx = frame_idx_ - 1;
	...
	else
	{
		...
		for (int det_id = 0; det_id < max_detections_count_; ++det_id) {
			...
			object.det_rect = cv::Rect(cv::Point(static_cast<int>(round(static_cast<double>(x0))),
				static_cast<int>(round(static_cast<double>(y0)))),
				cv::Point(static_cast<int>(round(static_cast<double>(x1))),
					static_cast<int>(round(static_cast<double>(y1)))));
